...
public class SummaryBulkAggregation<K, EV, S extends Serializable, T> extends SummaryAggregation<K, EV, S, T> {
	...
	public DataStream<T> run(final DataStream<Edge<K, EV>> edgeStream) {
		...
		DataStream<S> partialAgg = edgeStream
				...
				.aggregate(new PartialAgg<>(getInitialValue(), getUpdateFun(),returnType))
	...
	protected static final class PartialAgg<K, EV, S>
			implements ResultTypeQueryable<S>, AggregateFunction<Tuple2<Integer, Edge<K, EV>>, S, S> {

		private final S initialValue;
		private EdgesFold<K, EV, S> aggregateFunction;
		private TypeInformation<S> returnType;

		public PartialAgg(S initialValue, EdgesFold<K, EV, S> aggregateFunction, TypeInformation<S> returnType) {
...
//		}
		...
		@Override
		public S add(Tuple2<Integer, Edge<K, EV>> value, S accumulator) {
			try {
			return aggregateFunction.foldEdges(accumulator, value.f1.getSource(), value.f1.getTarget(), value.f1.getValue());
			} catch (Exception e) {
		...
		public S merge(S a, S b) {
			try {
				return aggregateFunction.foldEdges(a, null, null, (EV) b);
			} catch (Exception e) {
